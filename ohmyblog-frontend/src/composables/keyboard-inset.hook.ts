 ...
 * 小于该值一律视作没有键盘。
 *
 * 取 150 而不是几十，是被实测数据逼出来的（魅族 21 / Edge Android）：
 *   地址栏隐藏 innerHeight 765 → 显示 713，差 52
 *   键盘弹出   innerHeight 713 → 461，差 252
 * 阈值 40 时地址栏那 52px 就足以把 keyboardOpen 判成真 —— 键盘明明收着却被当成
 * 开着，插入面板据此选错了布局分支、渲染成 0 高度。两个量级差着 5 倍，把阈值放在
 * 中间地带才分得开。真实软键盘不会只有 150px 高，不必担心漏判。
 */
const NOISE_THRESHOLD_PX = 150;

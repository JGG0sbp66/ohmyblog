...
export const configRoute = new Elysia({ name: "configRoute" })
    .use(authPlugin).group("/config", { detail: { tags: ["Config (配置)"] } }, (app) =>
        app
            /**
             * PATCH /config
            ...
            .patch("/", async ({ body, set }) => {
                ...
                if (!data.configKey) {
                    throw new BusinessError("configKey 不能为空", { status: 400 });
                }
                if (data.configValue === undefined) {
                    throw new BusinessError("configValue 不能为空", { status: 400 });
                }
            ...
            .patch("/:configKey", async ({ params, body }) => {
                ...
                const hasPayload =
                    data.configValue !== undefined ||
                    data.description !== undefined ||
                    data.isPublic !== undefined;
                if (!hasPayload) {
                    throw new BusinessError("至少提供一个可更新字段", { status: 400 });
                }

                const updated = await configService.update(params.configKey, data);
                return {
                    message: "配置已更新",
                    config: updated,
                };
